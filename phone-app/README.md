...
```
while(true):
  0. 工作窗口判定(周一~五 08:00~20:00): 窗口外休眠, 每10分钟醒来看表, 零服务端额度
  1. outbox 补传(失败上报逐条重发)
   2. 距上次上报 ≥ 被动间隔(60min, 低电量≤20%×4 / ≤5%×8 省电, ≤5%时每6h刷新末位) → 定位+上报
  3. 短轮询挂线 GET /api/poll?wait=5 (X-Device-Key) → 强制间隙25s (占空比~15%, v2.4.1 免费额度省费)
     └ 收到命令 → 立即定位+上报(带cmd_id销单) → 地图约 0.5~1 分钟出新点
```
